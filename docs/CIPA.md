CIPA - Comissão interna de acidente


> NR MTE para consultar normas
## Objetivo da NR-05

> [!INFO] Estabelecer regras para
> - identificar riscos no ambiente de trabalho  
> - prevenir acidentes  
> - preservar a saúde dos trabalhadores
> 
> **Ou seja: a empresa não deve agir só depois do acidente — deve atuar preventivamente.**

## Riscos

- Risco Químico - Vermelho
- Risco Mecânico - Azul
- Risco Biológico - Marrom
- Risco Ergonômico - Amarelo
- Risco Físico - Verde

### Setor de Transporte
- Acidentes (colisões, tombamento ...)
- Quedas
- Esmagamento
- Risco elétrico (contato com rede elétrica)

![[Pasted image 20260218092901.png]]

>NR 06 - Para EPI's


![[Pasted image 20260218095557.png]]
.
Atividade
- Identificar todos os riscos visíveis
- Classifica-los
- sugerir medidas

**Risco Mecânico (Acidente)**
- Colisão com o veículo

**Risco Químico**
- Possível vazamento de diesel
- Risco de incêndio

**Medidas**
- Sinalização do local
- Motorista ficar fora da pista para evitar atropelamento

----
![[Pasted image 20260218133944.png]]


## Acidente Fictício

Veículo do tipo carreta baú, carregado, trafegava pela rodovia, em um trecho com curva acentuada, sob pista molha devido à chuva.
Ao entrar na curva o motorista perdeu o controle da veículo, onde ocorreu o tombamento do conjunto, bloqueando a pista parcialmente.
Motorista com excesso de jornada.

## Falhas Comportamento: 
- Velocidade incompatível com a pista molhada.
- Possível fadiga
## Falha de sinalização:
- Placa de sinalização de curva acentuada, estava parcialmente encoberta.
## ACIDENTE: Tombamento do caminhão  
-  Perda de estabilidade lateral  
	- Velocidade excessiva  
	- Pista molhada  
-  Falha na percepção do risco  
	- Sinalização insuficiente  
	- Fadiga do condutor  
# Causa Raiz Provável

> Velocidade inadequada sob pista molhada + fadiga
## Medidas
- Controle de jornada mais rigoroso;
- Treinamento/acompanhamento por instrutor;
- Avaliação periódica de desempenho do motorista