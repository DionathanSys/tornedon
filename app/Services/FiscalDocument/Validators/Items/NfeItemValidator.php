...
class NfeItemValidator
{
    ...
    public static function validate(array $data): array
    {
        ...
        $validator->after(function ($validator) use ($data): void {
            self::validateIbsCbsTaxData($validator, $data['items'] ?? []);
        });

        return $validator->validate();
    }

    private static function validateIbsCbsTaxData($validator, array $items): void
    {
        foreach ($items as $index => $item) {
            $ibsCbs = data_get($item, 'tax_data.imposto.ibs_cbs');
            ...
            $itemNumber = $index + 1;
            $requiredFields = [
                'situacao_tributaria' => 'A situação tributária IBS/CBS é obrigatória',
                'classificacao_tributaria' => 'A classificação tributária IBS/CBS é obrigatória',
                'grupo_ibs_cbs.valor_base_calculo' => 'A base de cálculo IBS/CBS é obrigatória',
                'grupo_ibs_cbs.ibs_estadual.aliquota' => 'A alíquota IBS estadual é obrigatória',
                'grupo_ibs_cbs.ibs_estadual.valor' => 'O valor IBS estadual é obrigatório',
                'grupo_ibs_cbs.ibs_municipal.aliquota' => 'A alíquota IBS municipal é obrigatória',
                'grupo_ibs_cbs.ibs_municipal.valor' => 'O valor IBS municipal é obrigatório',
                'grupo_ibs_cbs.cbs.aliquota' => 'A alíquota CBS é obrigatória',
                'grupo_ibs_cbs.cbs.valor' => 'O valor CBS é obrigatório',
            ];

            foreach ($requiredFields as $field => $message) {
                if (blank(data_get($ibsCbs, $field))) {
                    $validator->errors()->add(
                        "items.{$index}.tax_data.imposto.ibs_cbs.{$field}",
                        "{$message} no item {$itemNumber}."
                    );
                }
            }
        }
