...
class ServiceMigrationApiClient
{
    public function fetchPage(array $filters = []): array
    {
        ...
        $response = $request->get('/api/migracao/servicos', array_filter([
        ...
        if ($response->failed()) {
            throw new RuntimeException(sprintf(
                'Falha ao consultar API de migracao de servicos. HTTP %s. Resposta: %s',
                $response->status(),
                $response->body(),
            ));
        }
        ...
        if (! is_array($payload) || ! isset($payload['data'], $payload['meta'])) {
            throw new RuntimeException('Resposta invalida da API de migracao de servicos.');
        }
