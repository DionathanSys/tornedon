...
class AccountPayableInstallmentValidator
{
    ...
    private static function installmentUpdateRules(array $data): array
    {
        ...
        foreach (['paid_date', 'original_amount', 'interest_amount', 'fine_amount', 'discount_amount', 'paid_amount', 'balance_amount', 'bank_account_id', 'financial_category_id', 'cost_center_id', 'notes'] as $field) {
            array_unshift($rules[$field], 'sometimes');
        }
    ...
    private static function financialCategoryRule(array $data, string $scope, bool $nullable): array
    {
        ...
        $rules[] = function (string $attribute, mixed $value, \Closure $fail) use ($companyId, $scope): void {
            ...
            if (! $category->allows($scope)) {
                $fail('A categoria financeira selecionada nao pode ser usada em contas a pagar.');
            }
        };
