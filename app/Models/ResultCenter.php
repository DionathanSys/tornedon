    ...
    private function validateParent(): void
    {
        ...
        if ($this->exists && (int) $this->parent_id === (int) $this->id) {
            throw ValidationException::withMessages([
                'parent_id' => ['Um centro de resultado não pode ser pai dele mesmo.'],
            ]);
        }
        ...
        if (! $parent || (int) $parent->company_id !== (int) $this->company_id) {
            throw ValidationException::withMessages([
                'parent_id' => ['O centro de resultado pai deve pertencer a mesma empresa.'],
            ]);
        }
        ...
        while ($current) {
            ...
            if (isset($visited[$currentId])) {
                throw ValidationException::withMessages([
                    'parent_id' => ['A hierarquia de centros de resultado contem um ciclo.'],
                ]);
            }

            if ($this->exists && $currentId === (int) $this->id) {
                throw ValidationException::withMessages([
                    'parent_id' => ['O centro de resultado pai não pode ser descendente do centro atual.'],
                ]);
            }
