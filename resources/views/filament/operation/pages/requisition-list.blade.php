    ...
    <div class="op-queue">
        ...
        <div class="op-list">
            @forelse ($requisitions as $req)
                <a href="{{ $req['url'] }}" class="op-card" wire:navigate>
                    <div class="op-card__top">
                        <span class="op-card__number">{{ $req['number'] }}</span>
                        <span class="op-card__badge {{ $this->getStatusBadgeClass($req['status_value']) }}">{{ $req['status'] }}</span>
                    </div>
                    <p class="op-card__customer">{{ $req['customer'] }}</p>
                    <div class="op-card__meta">
                        <span class="op-card__meta-item">OS: <strong>{{ $req['service_order'] }}</strong></span>
                        <span class="op-card__meta-item">Itens: <strong>{{ $req['items_count'] }}</strong></span>
                        <span class="op-card__meta-item">{{ $req['sale_date'] }}</span>
                        <span class="op-card__meta-item"><strong>{{ $req['total'] }}</strong></span>
                    </div>
                </a>
            @empty
                <div class="op-empty">Nenhuma requisição encontrada.</div>
            @endforelse
        </div>
    </div>
</x-filament-panels::page>
