...
<body>
    <div class="header">
        ...
        <div class="subtitle">{{ $report['companyName'] }} | {{ $report['period'] }} | Gerado em {{ $report['generatedAt'] }}</div>
    ...
    <table class="report-table">
        ...
        @forelse ($report['rows'] as $row)
            <tr>
                @foreach ($report['columns'] as $column)
                    <td @class(['text-right' => in_array($column['name'], ['amount'], true)])>
                        {{ $row[$column['name']] }}
                    </td>
                @endforeach
            </tr>
        @empty
            <tr>
                <td colspan="{{ count($report['columns']) }}" class="text-center">Nenhum registro encontrado para o período selecionado.</td>
            </tr>
        @endforelse
